...
describe('Starting tests for rnet-ui', function() {
  ...
  it('website title should be rnet-ui', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('rnet-ui');
    })
  });
  ...
  it('navbar-brand should be rnet-ui',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('rnet-ui');
    });
  });
    ...
    it('Exporter table should have 5 columns',() => {
      page.navigateTo('/Exporter');
      element.all(by.css('.thead-cols th')).then(function(arr) {
    ...
    it('Importer table should have 5 columns',() => {
      page.navigateTo('/Importer');
      element.all(by.css('.thead-cols th')).then(function(arr) {
        expect(arr.length).toEqual(5); // Addition of 1 for 'Action' column
      });
    ...
    it('AccelerationReading component should be loadable',() => {
      page.navigateTo('/AccelerationReading');
      browser.findElement(by.id('transactionName'))
      .then((transactionName) => {
        return transactionName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('AccelerationReading');
      });
    });
